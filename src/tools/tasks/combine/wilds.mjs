...
const tempCrawlerRoot = 'temp/crawler/wilds'
const tempCombineRoot = 'temp/combine/wilds'
...
export const runAction = () => {
    ...
    // Load Raw Data
    Object.values(setting.wilds.targetList).forEach((target) => {
        rawDataMapping[target] = {}

        Object.values(setting.wilds.crawlerList).forEach((crawler) => {
    ...
    const mergeItem = (target, itemId, crawlerMapping) => {
        ...
        case 'armors':
            ...
            return Helper.deepCopy(item)
        case 'charms':
            item = Helper.deepCopy(dateset.charmItem)
...
export const infoAction = () => {
    ...
    for (let crawler of setting.wilds.crawlerList) {
        console.log(`count:${crawler}`)

        for (let target of setting.wilds.targetList) {
