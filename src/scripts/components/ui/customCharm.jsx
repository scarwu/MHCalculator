...
const handleRefreshCustomDataset = (majorData) => {
    if ('playerEquips' === majorData.target) {
        States.actions.setPlayerEquipCustomDataset('charm', majorData.custom)
    }

    if ('requiredConditions' === majorData.target) {
        States.actions.setRequiredConditionsEquipCustomDataset('charm', majorData.custom)
    }
}
...
export default function CustomCharm (props) {
    ...
    return useMemo(() => {
        ...
        const removeItem = () => {
            if ('playerEquips' === stateMajorData.target) {
                States.actions.setPlayerEquip('charm', null)
            }

            if ('requiredConditions' === stateMajorData.target) {
                States.actions.setRequiredConditionsEquip('charm', null)
            }
        }
                    ...
                    <div className="mhc-icons_bundle">
                        {('playerEquips' === stateMajorData.target && isNotRequire) ? (
                            <IconButton
                                ...
                                onClick={() => {
                                    States.actions.replaceRequiredConditionsEquipData('charm', stateMajorData)
                                }} />
                        ) : false}
                        <IconButton iconName="times" altName={_('clean')} onClick={removeItem} />
                    </div>
                                ...
                                <div className="col-3 mhc-value">
                                    <BasicSelector
                                        ...
                                        onChange={(event) => {
                                            ...
                                            // Clean Decoration
                                            if ('playerEquips' === stateMajorData.target) {
                                                States.actions.setPlayerEquipDecoration('charm', slotIndex, null)
                                            }
