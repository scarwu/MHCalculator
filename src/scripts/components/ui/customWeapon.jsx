...
const handleRefreshCustomDataset = (majorData) => {
    if ('playerEquips' === majorData.target) {
        States.actions.setPlayerEquipCustomDataset('weapon', majorData.custom)
    }

    if ('requiredConditions' === majorData.target) {
        States.actions.setRequiredConditionsEquipCustomDataset('weapon', majorData.custom)
    }
}
...
export default function CustomWeapon (props) {
    ...
    return useMemo(() => {
        ...
        const showModal = () => {
            States.actions.showModal('weaponSelector', {
        ...
        const removeItem = () => {
            if ('playerEquips' === stateMajorData.target) {
                States.actions.setPlayerEquip('weapon', null)
            }

            if ('requiredConditions' === stateMajorData.target) {
                States.actions.setRequiredConditionsEquip('weapon', null)
            }
        }
                    ...
                    <div className="mhc-icons_bundle">
                        {('playerEquips' === stateMajorData.target && isNotRequire) ? (
                            <IconButton
                                ...
                                onClick={() => {
                                    States.actions.replaceRequiredConditionsEquipData('weapon', stateMajorData)
                                }} />
                        ) : false}
                        <IconButton iconName="exchange" altName={_('change')} onClick={showModal} />
                        <IconButton iconName="times" altName={_('clean')} onClick={removeItem} />
                                ...
                                <div className="col-3 mhc-value">
                                    <BasicSelector
                                        ...
                                        onChange={(event) => {
                                            ...
                                            // Clean Decoration
                                            if ('playerEquips' === stateMajorData.target) {
                                                States.actions.setPlayerEquipDecoration('weapon', slotIndex, null)
                                            }
