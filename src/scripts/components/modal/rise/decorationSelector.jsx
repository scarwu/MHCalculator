...
const targetModalKey = 'decorationSelector'
...
const handleItemPickUp = (itemId, dataStore) => {
    if ('playerEquips' === dataStore.target) {
        States.rise.actions.setPlayerEquipDecoration(dataStore.equipType, dataStore.idIndex, itemId)
    }
}
 ...
 * Render Functions
 */
const renderDecorationItem = (decorationItem, dataStore) => {
    let classNames = [
        'mhc-item'
    ]

    if (Helper.isEmpty(dataStore.target) || decorationItem.id !== dataStore.id) {
            ...
            <div className="col-12 mhc-name">
                ...
                <div className="mhc-icons_bundle">
                    {Helper.isNotEmpty(dataStore.target) ? (
                        (decorationItem.id !== dataStore.id) ? (
                            <IconButton
                                iconName="check" altName={_('select')}
                                onClick={() => {
                                    handleItemPickUp(decorationItem.id, dataStore)
                                }} />
                        ) : (
                            <IconButton
    ...
    useEffect(() => {
        ...
        if (Helper.isNotEmpty(dataStore.target)) {
            ...
            if ('playerEquips' === dataStore.target
                ...
                && Helper.isNotEmpty(_playerEquips[equipType].decorationIds)
                && Helper.isNotEmpty(_playerEquips[equipType].decorationIds[idIndex])
            ) {
                dataStore.id = _playerEquips[equipType].decorationIds[idIndex]
            }
        }
